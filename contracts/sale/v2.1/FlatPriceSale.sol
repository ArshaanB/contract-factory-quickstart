...
contract FlatPriceSale_v_2_1 is Sale, PullPaymentUpgradeable {
...
// 	) public initializer validUpdate(_config) {
  ...
  */
	function _settleNativeToken(uint256 baseCurrencyValue, uint256 nativeTokenQuantity) internal {
		uint256 nativeFee = 0;
		if (feeBips > 0) {
			nativeFee = (nativeTokenQuantity * feeBips) / fractionDenominator;
  ...
  */
   ...
   */
	function buyWithNative(bytes calldata data, bytes32[] calldata proof)
		external
		payable
		override
